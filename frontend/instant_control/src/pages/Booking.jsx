  ...
  useEffect(() => {
    ...
    <div className="w-full h-full text-primary p-5 block lg:absolute top-0 left-0">
        ...
        <div className="w-full p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-5 place-items-center">
            {tickets.length > 0 ? (
                tickets.map(ticket => (
                <div className="flex items-center justify-around p-5 bg-primary rounded-lg hover:shadow-xl hover:-translate-y-2 hover:duration-100 w-full h-full overflow-hidden" key={ticket._id}>
                ...
                <div className="text-sm md:text-base ml-5 text-white">
                    <p>Date/Time: <b className="text-secondary"> {ticket.date}</b></p>
                    <p>Total: <b className="text-secondary"> $20</b></p>
